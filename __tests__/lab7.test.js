describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
    const findProductItem = await page.$('product-item');
    const shadowRoot = await findProductItem.getProperty('shadowRoot');
    const shadowButton = await page.evaluateHandle(
      root => root.querySelector('button'),
      shadowRoot
    )
    await shadowButton.click();
    const text = await shadowButton.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 1; i < allProductItems.length; i++) {
      const shadowButton = await page.evaluateHandle(
